...
from infra.kafka_producer import produce_contest_end
...
def contest_thread(participate: Participate, current_data):
    ...
    def cal_now_stock_cost():
        sell_sum = (session_thread.query(func.sum(Decimal(Trade.trade_cnt) * Decimal(Trade.trade_cost)))
                    .filter(Trade.participate_id == participate.id, Trade.trade_type == "sell")
                    .one()[0] or Decimal(0))

        buy_sum = (session_thread.query(func.sum(Decimal(Trade.trade_cnt) * Decimal(Trade.trade_cost)))
    ...
    def buy(param: int):
        ...
        if param * current_data['5'] < participate.result_money:
    ...
    def asset(percent):
        ...
        return math.ceil(tmp_asset / current_data['5'])
...
async def check_contest():
    ...
    if contest:
        ...
        await start_contest(session, contest_info=cur_contest, participates=participates)
...
async def start_contest(session,
    ...
    while datetime.now() < contest_info.end_time:
        ...
        for participate in participates:
            ...
            _thread.start_new_thread(contest_thread, (participate, current_data))
    ...
    for participate in participates:
        # buy 개수만큼 팔기
        session.query(Participate).filter(Participate.member_id == participate.member_id). \
            update({Participate.result_money: Participate.result_money +
                                              session.query(func.sum(Trade.trade_cost * Trade.trade_cnt)).
                   filter(Trade.participate_id == Participate.id).scalar()})
